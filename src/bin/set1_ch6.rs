...
pub fn main() {
    ...
    let mut xord_buff: Vec<Vec<xor::helpers::XorResult>> = Vec::new();
    ...
    for o in optimal_key_block_size {
        ...
        for x in 0..transposed.len() {
            let xor: Vec<xor::helpers::XorResult> = xor::helpers::single_char_xor(o, x, &transposed[x]);
    ...
    let answer = xor::helpers::solve_repeating_xor(&key.chars().collect(), &enc_str);
...
pub fn filter_results(results: Vec<xor::helpers::XorResult>) -> Vec<xor::helpers::XorResult> {
...
fn find_ideal_key_size(min_key_size: usize, max_key_size: usize, enc_str: &Vec<u8>) -> Vec<KeySizeResult>{
    ...
    // while min_key_size <= max_key_size {
        ...
        let mut first: Vec<u8> = vec![0; key_size];
        let mut second: Vec<u8> = vec![0; key_size];
        let mut third: Vec<u8> = vec![0; key_size];
        let mut fourth: Vec<u8> = vec![0; key_size];

        first.copy_from_slice(&enc_str[0..key_size]);
        second.copy_from_slice(&enc_str[key_size..key_size * 2]);
        third.copy_from_slice(&enc_str[key_size * 2..key_size * 3]);
        fourth.copy_from_slice(&enc_str[key_size * 3..key_size * 4]);

        //sum the hammings distance between them and devide by key_size
        hammings_total += (
            text_utils::hamming_distance(&first, &second) + 
            text_utils::hamming_distance(&second, &third) +
            text_utils::hamming_distance(&third, &fourth)) / 3;
...
mod tests {
    ...
    #[test]
    pub fn set1_ch6_test_collect_cypher() {
        ...
        for (i, t) in transposed.iter().enumerate() {
            xor::helpers::single_char_xor(3, i, &t);
        }
    }

    #[test]
    pub fn set1_ch6_test_decrypt_with_key() {
        let file_name = "challenge_files/6.txt";
        ...
        let answer = xor::helpers::solve_repeating_xor(&key, &enc_str);
        let decrypt: String = answer.iter().map(|x| *x as char).collect();
