...
fn base64_encode_hex_vec(buf: Vec<u32>) -> String {
    let mut buf_size = 0;
    ...
    for b in buf {
        ...
        if buf_size == 6 {
            let encoding = base64_match(byte_buf, buf_size/2);
            // dbg!(&encoding);
...
pub fn base64_encode_hex_str(buf: &'static str) -> String {
    let mut buf_size = 0;
    let mut byte_buf: u32 = 0;
    let mut encoded: String = String::new();
...
fn base64_encode_str(val: &str) -> String {
    ...
    for c in val.chars()  {
        ...
        if buf_size == 3 { 
            let encoding = base64_match(byte_buf, buf_size);
            encoded.push_str(&encoding);
            byte_buf = 0;
            buf_size = 0;
            continue;
...
fn base64_match(mut byte_buf: u32, buf_size: u8) -> String {
    let mut encoded: String = String::new();
    let mut pos = 0;
...
mod tests {
    ...
    pub fn test_base64_match() {
        ...
        assert_eq!(base64_match(input, 3), "TWFu");
    }

    #[test]
    pub fn test_hex_decode() {
        assert_eq!(base64_encode_hex_str(VAL), RESULT);
    }

    #[test]
    pub fn test_manual_encode() {
        dbg!(base64_match(0b_00100000_01100100, 2));
    }

    #[test]
    pub fn test_run() {
        assert_eq!(base64_encode_hex_str(VAL), RESULT);
        assert_eq!(base64_encode_str(T1), T1_ANSWER);
    }
    ...
    pub fn test_run_2() {
        let test_val = ("s", "cw==");
        dbg!(test_val);
        assert_eq!(base64_encode_str(test_val.0),test_val.1);
        let test_val = ("Ma", "TWE=");
        dbg!(test_val);
        assert_eq!(base64_encode_str(test_val.0),test_val.1);
