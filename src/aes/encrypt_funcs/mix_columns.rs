use crate::aes::helper::{overflow_check, xy_idx};
use crate::aes::tables as tbl;

pub fn poly_mix(state: Vec<u8>) -> Vec<u8> {
    ...
    while s_pos < state.len() as i32 {
        ...
        t_state[xy_idx(0, y)] = operate(&state, y, col);
        col += 1;
        t_state[xy_idx(1, y)] = operate(&state, y, col);
        col += 1;
        t_state[xy_idx(2, y)] = operate(&state, y, col);
        col += 1;
        t_state[xy_idx(3, y)] = operate(&state, y, col);
        col += 1;
...
fn operate(state: &Vec<u8>, y: i32, col: usize) -> u8 {
    let t1 = overflow_check(state[xy_idx(0, y)], tbl::m_mtrx(0, col));
    let t2 = overflow_check(state[xy_idx(1, y)], tbl::m_mtrx(1, col));
    let t3 = overflow_check(state[xy_idx(2, y)], tbl::m_mtrx(2, col));
    let t4 = overflow_check(state[xy_idx(3, y)], tbl::m_mtrx(3, col));
    t1 ^ t2 ^ t3 ^ t4
}
...
pub fn table_mix(state: Vec<u8>) -> Vec<u8> {
    ...
    while s_pos < state.len() as i32 {
        ...
        let t1 = overflow_check(tbl::l_box(state[xy_idx(x + 0, y)]), tbl::l_box(tbl::m_mtrx(m_row, m_col)));
        let t2 = overflow_check(tbl::l_box(state[xy_idx(x + 1, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 1, m_col)));
        ...
        let t2 = overflow_check(tbl::l_box(state[xy_idx(x + 1, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 1, m_col)));
        let t3 = overflow_check(tbl::l_box(state[xy_idx(x + 2, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 2, m_col)));
        let t4 = state[xy_idx(x + 3, y)];
        ...
        let t3 = overflow_check(tbl::l_box(state[xy_idx(x + 2, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 2, m_col)));
        let t4 = overflow_check(tbl::l_box(state[xy_idx(x + 3, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 3, m_col)));
        t_state[xy_idx(x + 2, y)] = t1 ^ t2 ^ tbl::e_box(t3) ^ tbl::e_box(t4);
        // println!("3 -- t_state: {:02x} at {},{}", t_state[xy_idx(x, y + 2)], x, y);
        
        m_col += 1;
        let t1 = overflow_check(tbl::l_box(state[xy_idx(x, y)])     , tbl::l_box(tbl::m_mtrx(m_row, m_col)));
        let t2 = state[xy_idx(x + 1, y)];
        let t3 = state[xy_idx(x + 2, y)];
        let t4 = overflow_check(tbl::l_box(state[xy_idx(x + 3, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 3, m_col)));
