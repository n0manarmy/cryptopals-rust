...
pub fn mix(state: Vec<u8>) -> Vec<u8> {
    ...
    while s_pos < state.len() as i32 {
        ...
        let t1 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 0, y)]), tbl::l_box(tbl::m_mtrx(m_row, m_col)));
        let t2 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 1, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 1, m_col)));
        let t3 = state[t_xy_idx(x + 2, y)];
        let t4 = state[t_xy_idx(x + 3, y)];
        t_state[xy_idx(x, y)] = tbl::e_box(t1) ^ tbl::e_box(t2) ^ t3 ^ t4;
        // println!("1 -- t_state: {:02x} at {},{}", t_state[xy_idx(x, y)], x, y);
        
        m_col += 1;
        let t1 = state[t_xy_idx(x + 0, y)];
        let t2 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 1, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 1, m_col)));
        let t3 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 2, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 2, m_col)));
        let t4 = state[t_xy_idx(x + 3, y)];
        ...
        m_col += 1;
        let t1 = state[t_xy_idx(x + 0, y)];
        let t2 = state[t_xy_idx(x + 1, y)];
        let t3 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 2, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 2, m_col)));
        let t4 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 3, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 3, m_col)));
        t_state[xy_idx(x + 2, y)] = t1 ^ t2 ^ tbl::e_box(t3) ^ tbl::e_box(t4);
        // println!("3 -- t_state: {:02x} at {},{}", t_state[xy_idx(x, y + 2)], x, y);
        
        m_col += 1;
        let t1 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x, y)])     , tbl::l_box(tbl::m_mtrx(m_row, m_col)));
        let t2 = state[t_xy_idx(x + 1, y)];
        let t3 = state[t_xy_idx(x + 2, y)];
        let t4 = l_box_overflow_check(tbl::l_box(state[t_xy_idx(x + 3, y)]) , tbl::l_box(tbl::m_mtrx(m_row + 3, m_col)));
