...
pub fn l_box(val: u8) -> u8 {
    ...
    let xy_idx = box_xy_idx(val);
    // println!("l_box val {:02x}", val);

    let l_box: [[u8; 16]; 16] =
    [
        [ 0x0, 0x00, 0x19, 0x01, 0x32, 0x02, 0x1A, 0xC6, 0x4B, 0xC7, 0x1B, 0x68, 0x33, 0xEE, 0xDF, 0x03],
...
fn box_xy_idx(val: u8) -> (usize, usize) {
    ...
    pos.0 = hex::encoders::translate_char_to_hex_val(val[0]); 
    pos.1 = hex::encoders::translate_char_to_hex_val(val[1]); 
